...
export function OptionWheel({
  ...
  const initialIndex = controlledSelected ?? defaultSelected;
  ...
  const runFrame = useCallback((now: number) => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (R > 0) {
        ...
        y = R * Math.sin(ang);
        // Positive x curve inwards to the right when side is left
    ...
    >
      {items.map((label, index) => (
        <div
          ...
          className={`absolute ${
            align === "top" ? "top-7" : "top-1/2"
          } cursor-pointer whitespace-nowrap leading-none transition-colors will-change-[transform,opacity,filter] [font-size:var(--ow-font-size)] [color:color-mix(in_srgb,var(--ow-active-color)_calc(var(--ow-p,0)*100%),var(--ow-text-color))] ${
            side === "right"
              ? "right-[var(--ow-inset)] origin-right"
              : "left-[var(--ow-inset)] origin-left"
          } ${selectedIndex === index ? "font-semibold tracking-tight" : "font-light"}`}
